    ...
    <main>
        ...
        <transition name="bounce">
            <div
                v-if="activePlayer"
                class="info-bar"
                @animationstart="newTurnSound"
            >
                <b class="h4"> {{ activePlayer.name }}&nbsp;</b>
...
export default Vue.extend({
    ...
    mounted() {
        connection.room?.onMessage("attacked", (d: IAttackBroadcast) => {
            ...
            const animationQueue = Object.entries(d.attack)
                .filter(([_k, v]) => typeof v === "number" && v > 0)
                .map(([k]) => k) as unknown as (keyof IAttack)[];
            // Play effects
            const effectsPlayer: any = setInterval(function () {
                // Exit if all animations have finished
                if (
                    animationQueue.length === 0 ||
                    !connection.unsynced.lastAttack?.attack
                )
                    return clearInterval(effectsPlayer);
                // Get key of effect in audio/attack
                const effectType: keyof IAttack = animationQueue.splice(
                    0,
                    1
                )[0];
                // Play the audio
                ...
                if (connection.unsynced.lastAttack) {
                    (connection.unsynced.lastAttack.attack[effectType] as any) =
                        d.attack[effectType] as any;
                }
            }, 1000);
